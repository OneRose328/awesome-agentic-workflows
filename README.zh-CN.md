...
- `PATCH`：修复描述、文档和兼容性细节，不改变模板内容。
- `MINOR`：新增模板或有意义地扩展现有工作流。
- `MAJOR`：结构或兼容性假设发生破坏性变更。
- GitHub Agentic Workflows 有变动时，先核对官方文档，在 [CHANGELOG.md](CHANGELOG.md) 中记录兼容性更新，再在下次发版说明中体现。
- 更新已在使用中的模板前，先更新 `.md` 源文件，重新运行 `gh aw validate --dir templates/<分类>`，编译受影响的示例工作流后再合并。
